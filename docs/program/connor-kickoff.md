...
**生命周期**：此处生命周期是指，用户在使用本产品时，从进入的 **初始态** 到完成的 **终态** 之间所有相关事物（产品形态和用户动作）集合。**Connor** 对此进行高度的抽象简化。
  ...
  -   **执行**：为了进一步提升用户的 **感知** 和 **使用体验**，**Connor** 在执行阶段设计有大量的用户交互动作，以增强用户使用体感和对本产品的依赖。秉持着以 **用户动作** 代替 **用户思考** 的设计思想，**Connor** 参照日常行为模式，从用户真实需求出发，将复杂的思考和输入过程简化成日常操作，以 `引导`、`选择`、`确认`、`点击` 等方式建立起来用户的行为链条。**执行** 阶段整体 [流程图](./connor-kickoff.md#135-connor-执行流程) 如下。
      ...
      |<span style="color: #999">2</span>|任意当日日程到达开始执行前 **15min**|触发执行提醒和操作弹窗|1. 提醒用户即将开始日程。<br/>2. 提醒的卡片信息包含以下内容：<br/><ul><li>头部为提醒卡片标题：您有任务即将开始执行</li><li>下部为具体需要执行的任务信息，包含：任务名称、起止时间（hh:mm ~ hh:mm）、任务详情和任务进度等信息。</li><li>底部为操作按钮：执行、调整。点击【执行】按钮，任务状态流转到进行中，点击【调整】按钮，进入调整二级页面操作，保存后会重新生成当前的日程。</li></ul>|
      |<span style="color: #999">3</span>|任意当日日程结束前|任务卡片操作弹窗|1. 点击结束按钮，结束当前日程。手动结束会进入二级页面，展示日程实际开始时间（点击开始执行按钮时间）和点击结束按钮时间，根据时间跨度比例计算日程完成度，用户可以自行修改进度，最大为100%。<br />2. 当前日程达到结束时间或下一日程开始，当前日程自动结束，根据执行时间跨度自动计算日程完成进度，可在当天所有日程结束后的总结卡片中进入调整二级页面，修改当天所有的日程进度。<br />3. 当天24点后，不允许再修改日程相关信息。此时会将日程相关执行同步至任务列表。|
 
  -   **复盘**：复盘主要包含以下三个板块内容，**报告推送**、**执行详情** 和 **智能分析**。**报告推送:** 由于前两个生命周期中，app 收集到了较多的用户操作数据，根据内置<span style="color: #999; font-weight: 300; font-style: italic">（考虑到拓展性，部分功能开放支持用户自定义设置）</span>策略生成复盘信息，通过周、月、年等 **时间维度** 和 **目标维度** 推送给用户，实现准确、快速、及时的执行状况反馈，帮助用户更好得达成目标。**执行详情:** 以目标、任务维度展示用户的执行细节，包含任务执行进度，是否延期、目标完成度、里程碑是否点亮等。**智能分析（<span style="color: orange">高级功能</span>）:** 根据 **目标**、**时间** 维度分析用户的执行习惯，分析行为模式中可提升的点（考虑融入AI）。所有的复盘功能主要以 **图表**📈 加 *少量文字* 的方式展示。
...
-   **交互设计**：具体详见 [交互稿](./connor-kickoff.md#16-原型图)、[产品流程](./connor-kickoff.md#14-产品整体流程) 和使用文档。
...
WAITING@{ shape: hourglass, label: "Collate" }
  subgraph timeLine[时间轴]
    ...
    subgraph endDay[调整每日日程执行情况]
      direction TB
      detailPage[每日日程调整页面] --> syncTasks[同步任务进度]
    end
    alarm2 --> |点击调整按钮|endDay
  end
FC e1@--> |每日零点| timeLine
timeLine --> END
e1@{ animate: true }
...
**Connor** 是一个目标 🎯 实现App，核心能力在于通过简单、直观的指令，帮助用户实现更高效的目标达成。

<span style="color: #999; font-weight: 300; font-style: italic">使用流程待原型设计完成后补充...</span>
...
block-beta
  ...
  block:group2:3
    columns 3
    DAILY["日程下发"] ALRAM["执行提醒"]
    TIMER["专注时间"] STATISFICATION["进度统计"] QUICK["快捷操作"] ETC["..."] 
  end
  CONDUCT["执行"]:1
  block:group1:3
    columns 3
    GOAL["目标"] TASK["任务"] NODE["里程碑"]
  end
  IO["立项"]:1
